...
func _on_body_entered(body):
	if body.name == "Sonic" and not en_recorrido and path_follow:
		sonic_ref = body
		en_recorrido = true
...
func _process(delta):
	if en_recorrido and sonic_ref:
		...
		if anim_node: anim_node.rotation = path_follow.rotation
		if path_follow.progress_ratio >= 0.99: finalizar()
